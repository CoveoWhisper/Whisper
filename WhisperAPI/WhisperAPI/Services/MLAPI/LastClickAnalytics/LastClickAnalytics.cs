...
namespace WhisperAPI.Services.MLAPI.LastClickAnalytics
{
    public class LastClickAnalytics : ILastClickAnalytics
    {
        ...
        public async Task<List<LastClickAnalyticsResults>> GetLastClickAnalyticsResults(HashSet<string> contextEntities)
        {
            var response = await this._httpClient.PostAsync("ML/Analytics", CreateStringContent(contextEntities));
            response.EnsureSuccessStatusCode();

            return JsonConvert.DeserializeObject<List<LastClickAnalyticsResults>>(response.Content.ReadAsStringAsync().Result);
        }
